# title: "Straddle Study"
# author: "Jason Taylor"

# Todos:
# - Change the custom open dates from csv to save maybe?
straddle <- function(progress.int, t) {
  shiny::withProgress(message = "Progress Bar", detail = "Opening Trades", {
    ...
    for (i in unique(first.day$open.date))  {
      ...
      if (nrow(start.data) > 0 && start.data[1, iv.rank] >= low.iv &&
        ...
        put.data <- dplyr::filter(put.data, dte.diff == min(dte.diff)) # Closest to DTE
        put.data <- dplyr::filter(put.data, dte == max(dte)) # Max DTE if there are two same diff from target DTE
        put.data <- dplyr::filter(put.data, call.put == "P")
        put.data <- dplyr::filter(put.data, strike >= price)
        put.trade <- dplyr::filter(put.data, strike == min(strike))
        call.data <- dplyr::filter(start.data, dte == put.trade$dte)
        call.data <- dplyr::filter(call.data, call.put == "C")
        call.trade <- dplyr::filter(call.data, strike == put.trade$strike)
  ...
  shiny::withProgress(message = "Progress Bar", detail = "Closing Trades", value = .5, {
    ...
    for (i in 1:nrow(c.positions))  {
      ...
      put.close <- dplyr::filter(trade.data,
                                 date == e, expiration == e,
                                 strike == s, call.put == "P")
      ...
      call.close <- dplyr::filter(trade.data,
                                  date == e, expiration == e,
                                  strike == s, call.put == "C")
    ...
    for (i in 1:nrow(put.results))  {
      ...
      this.case <- as.data.frame(matrix(nrow = 1,
                                        c(t, ps, od, cd, d, pop, iv, rsi, pp, cp,
                                          cs, cop, tp, er)),
  ...
  shiny::withProgress(message = "Progress Bar", detail = "Creating Plot", value = .95, {
    ...
    results.table <- dplyr::select(results,
                                   ...
                                   open.ivrank, put.strike, call.strike,
                                   exit.reason, profit)
